...
const createDatabasePool = async () => {
  ...
  const pool = mysql.createPool({
    ...
    user: isProduction ? process.env.PROD_HYBRID_USER : process.env.DEV_HYBRID_USER,
    password: isProduction ? process.env.PROD_HYBRID_PASS : process.env.DEV_HYBRID_PASS,
