...
const createDatabasePool = async () => {
  ...
  const pool = mysql.createPool({
    ...
    user: isProduction ? process.env.PROD_SUPERADMIN_USER : process.env.DEV_SUPERADMIN_USER,
    password: isProduction ? process.env.PROD_SUPERADMIN_PASS : process.env.DEV_SUPERADMIN_PASS,
