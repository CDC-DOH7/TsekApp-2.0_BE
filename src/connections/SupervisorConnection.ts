...
const createDatabasePool = async () => {
  ...
  const pool = mysql.createPool({
    ...
    user: isProduction
      ? process.env.PROD_SUPERVISOR_USER
      : process.env.DEV_SUPERVISOR_USER,
    password: isProduction
      ? process.env.PROD_SUPERVISOR_PASS
      : process.env.DEV_SUPERVISOR_PASS,
    ...
    ssl: isProduction
      ? {
          ca: fs.readFileSync(
            String(process.env.CA_CERTIFICATE_PATH)
          ),
